...
public class TUIManager : CSingleton<TUIManager>
{
    ...
    public bool RegisterUI(string aDir, string aUIName)
    {
        if (this.mUIDic.ContainsKey(aUIName) == true)
        {
            Debug.LogError("The key of registered UI was existed!");
            return false;
        }
        GameObject prefab = Resources.Load<GameObject>(aDir + aUIName);
        if (prefab == null)
        {
            Debug.LogError("Can't find the UI prefab!");
            return false;
        }
        this.mUIDic.Add(aUIName, prefab);
        return true;
    }
